    ...
    <>
      {BLOCKS.map((block) => (
        <section 
          key={block.id} 
          id={block.id}
          className={`h-screen w-full relative overflow-hidden snap-start flex flex-col justify-end p-5 md:p-12 transition-colors duration-1000 ${block.bgColor}`}
        >
          ...
          {/* Content */}
          <div className={`relative z-10 max-w-screen-2xl mx-auto w-full border-l-2 border-current pl-6 md:pl-16 ${block.textColor}`}>
            ...
            <h2 className="text-[14vw] md:text-[12vw] font-extrabold tracking-tighter mb-8 md:mb-12 leading-[0.75] uppercase">
              {block.title}
            </h2>
            
            <div className="max-w-xl">
               <p className="opacity-70 text-lg md:text-xl font-sans tracking-tight leading-relaxed mb-8 md:mb-12">
                {block.description}
              </p>
              <button 
                onClick={() => onSelectCategory(block.category)}
                className="group flex items-center gap-4 font-bold text-[11px] md:text-sm tracking-[0.3em] uppercase cursor-pointer"
              >
                Explore Projects
