...
export default function App() {
  ...
  const bounceItems = useMemo(() => [
    ...
    { id: 'editorial-bounce', label: 'EDITORIAL', onClick: () => {
      setSelectedCategory('Editorial');
      document.getElementById('editorial')?.scrollIntoView({ behavior: 'smooth' });
    }},
    { id: 'techpack-bounce', label: 'TECHPACKS', onClick: () => {
      setSelectedCategory('Techpack');
      document.getElementById('techpacks')?.scrollIntoView({ behavior: 'smooth' });
    }},
  ], []);
  ...
  return (
    <main className="h-screen overflow-y-scroll snap-y snap-mandatory bg-off-white selection:bg-electric-blue selection:text-white">
      ...
      <section 
        id="home" 
        className="h-screen w-full snap-start flex flex-col pt-32 md:pt-32 px-4 md:px-12 relative overflow-hidden bg-off-white"
